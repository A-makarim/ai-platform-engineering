...
describe('chat-store', () => {
  ...
  describe('loadMessagesFromServer', () => {
    ...
    it('preserves autonomous synth messages when MongoDB returns only user-typed turns', async () => {
      // Autonomous synth messages (creation_intent, run_request, run_response,
      // next_run_marker) are generated client-side from autonomous-service
      // data and never persisted in MongoDB. Without the autonomous-aware
      // union the server's user-typed-only response would clobber them on
      // every loadMessagesFromServer cycle, producing the
      // "manual chat history disappears in autonomous task chat — switch
      // away and back twice before it shows" symptom (Inv-G addendum).
      const autonomousId = 'auto-task-1';
      const synthCreation = makeMessage({
        id: `creation-intent-${autonomousId}`,
        role: 'user',
        content: 'List GitHub PRs every morning',
        timestamp: new Date('2026-05-13T08:00:00.000Z'),
      });
      const synthRunRequest = makeMessage({
        id: 'run-request-run-1',
        role: 'user',
      ...
      const liveAutonomous = makeConversation({
        ...
        title: 'Cron task',
        source: 'autonomous',
        messages: [synthCreation, synthRunRequest, userTyped],
      });
      useChatStore.setState({ conversations: [liveAutonomous] });

      // MongoDB has only the user-typed message — synth messages are
      // never persisted there.
      mockApiClient.getMessages.mockResolvedValue({
        items: [
          {
            _id: 'mongo-followup',
            message_id: 'user-followup-1',
            conversation_id: autonomousId,
            role: 'user',
            content: 'follow up',
      ...
      await useChatStore.getState().loadMessagesFromServer(autonomousId);

      const updated = useChatStore
        .getState()
        .conversations.find((c) => c.id === autonomousId);
      expect(updated).toBeDefined();
      // All three messages survive: 2 synth (client-only) + 1 user-typed (MongoDB).
      const ids = updated!.messages.map((m) => m.id);
      expect(ids).toContain(`creation-intent-${autonomousId}`);
      expect(ids).toContain('run-request-run-1');
      expect(ids).toContain('user-followup-1');
      expect(updated!.messages).toHaveLength(3);
      // Sorted by timestamp.
      expect(updated!.messages[0].id).toBe(`creation-intent-${autonomousId}`);
      expect(updated!.messages[1].id).toBe('run-request-run-1');
      expect(updated!.messages[2].id).toBe('user-followup-1');
    });
