...
4. **Inv-G follow-up — `loadMessagesFromServer` synth preservation**: After (3) shipped, the symptom moved one layer down: when a user opened an autonomous chat, `ChatContainer` calls `loadMessagesFromServer(uuid)` to refresh from MongoDB. The non-streaming branch built `mergedMessages` by `messages.map((serverMsg) => ...)` over the **server's** array — any local message whose id is not in the server response (i.e. every autonomous synth message: `creation-intent-*`, `preflight-ack-*`, `run-request-*`, `run-response-*`, `next-run-marker-*`, all generated by `synthesizeConversationForTask`) was silently dropped. The visible effect: open the autonomous chat post-refresh → server loader writes user-typed-only messages → the user has to switch away and back **twice** before the autonomous-loader's 30 s sidebar tick re-injects the synth messages. The non-streaming merge in `loadMessagesFromServer` now unions local-only messages back in when `existingConv.source === 'autonomous'`, sorted by `timestamp` (NaN-safe — non-finite timestamps sort to the end, matching Inv-A's rehydrate winner-selection rule).
