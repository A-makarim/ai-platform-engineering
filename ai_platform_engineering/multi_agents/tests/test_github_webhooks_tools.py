...
"""Tests for supervisor-side GitHub webhook management tools.

The tools wrap GitHub's repository webhook API and should return readable
...
def _stub_token(monkeypatch):
    """Give each test a fake PAT unless it explicitly unsets the env var."""
...
def test_register_uses_caller_supplied_secret_and_events(monkeypatch):
    ...
    # Caller-supplied secrets must not be echoed into logs or chat history.
