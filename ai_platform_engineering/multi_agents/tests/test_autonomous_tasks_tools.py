...
"""Tests for supervisor-side autonomous task tools.

The tools wrap the autonomous-agents REST API and should return readable
strings for both success and failure paths.
"""
...
def _patch_httpx(monkeypatch, handler):
    """Route tool HTTP clients through an ``httpx.MockTransport`` handler."""
...
def test_list_returns_summary_when_tasks_exist(monkeypatch):
    ...
    assert "`t1`" in result
    assert "`t2`" in result
    # Task t2 has no agent, so render the LLM-routed marker
...
def test_create_cron_without_schedule_returns_validation_error(monkeypatch):
    """Cron triggers require ``trigger_schedule`` before any HTTP call."""
...
# ---------------------------------------------------------------------------
# create_autonomous_task - webhook auto-secret + callback URL
# ---------------------------------------------------------------------------


def test_create_webhook_task_auto_generates_secret(monkeypatch):
    """Auto-generate a webhook secret and surface it for the next tool call."""
    ...
    assert "/api/v1/hooks/auto-triage" in result
    assert secret_sent in result  # the exact secret is echoed once
    assert "auto-generated" in result
    # Warn when the generated callback URL is probably not externally reachable.
...
def test_create_webhook_task_uses_caller_supplied_secret_without_echo(monkeypatch):
    """Forward caller secrets to the server without echoing them back."""
    monkeypatch.delenv("AUTONOMOUS_AGENTS_PUBLIC_URL", raising=False)
    captured: dict = {}
    ...
    # callback_url is still surfaced; it is not sensitive.
    assert "callback_url:" in result


def test_create_webhook_task_uses_public_url_when_set(monkeypatch):
    """Use AUTONOMOUS_AGENTS_PUBLIC_URL when building callback URLs."""
    monkeypatch.setenv(
    ...
    assert "callback_url: https://abcd-1-2-3-4.ngrok.io/api/v1/hooks/demo" in result
...
def test_create_cron_task_does_not_include_webhook_fields(monkeypatch):
    """Non-webhook tasks must not include webhook-only response fields."""
    captured: dict = {}
...
def test_create_webhook_task_accepts_public_url_with_api_prefix(monkeypatch):
    monkeypatch.setenv(
        "AUTONOMOUS_AGENTS_PUBLIC_URL", "https://abcd-1-2-3-4.ngrok.io/api/v1"
    )

    def handler(_request):
        return httpx.Response(
            201,
            json=_ok_task("demo", trigger={"type": "webhook", "has_secret": True}),
        )
...
def test_create_omits_optional_fields_from_payload(monkeypatch):
    """Omit server-optional fields instead of sending explicit nulls."""
    captured = {}
...
def test_validate_cron_rejects_garbage_field():
    """Bad cron content should not crash validation."""
...
def test_autonomous_agents_url_honours_env(monkeypatch):
    ...
    # Strip trailing slash so composed URLs do not contain double slashes.
    # in composed URLs.
