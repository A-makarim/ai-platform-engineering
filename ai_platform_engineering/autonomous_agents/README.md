...
For GitHub or other external senders, set `AUTONOMOUS_AGENTS_PUBLIC_URL`
to the public service host, for example `https://example.ngrok.app`.
Supervisor tools append `/api/v1/hooks/{task_id}` when they generate
callback URLs. Existing deployments that include `/api/v1` in the public
base URL are still accepted.
